...
const DarbarBookingPage = () => {
  ...
  // ✅ Toast on success
  useEffect(() => {
    if (successMessage && createdTokenNumber) {
      toast.success(`Token #${createdTokenNumber} booked successfully!`, { duration: 4000 });
    }
  }, [successMessage, createdTokenNumber]);
  ...
  useEffect(() => {
    ...
    checkBookingOpen();
    const interval = setInterval(checkBookingOpen, 60000);
    return () => clearInterval(interval);
  }, [dispatch]);

  const checkBookingOpen = () => {
    const now = moment().utcOffset("+05:30");
    const open = (now.day() === 6 && now.hour() >= 16) || (now.day() === 0 && now.hour() < 13);
      ...
      <div className="max-w-xl w-full space-y-10 bg-white p-10 md:p-12 rounded-[3rem] shadow-2xl border border-gray-100">
        ...
        {!isBookingOpen ? (
          <div className="bg-orange-50 border border-orange-100 p-6 rounded-3xl text-center">
            ...
            <p className="text-orange-800/70">Booking opens Saturday 4:00 PM – Sunday 1:00 PM.</p>
          </div>
          ...
          <div className="bg-white border border-gray-100 p-10 rounded-[2.5rem] text-center shadow-xl">
            ...
            <p className="text-gray-500 mb-8 leading-relaxed">
              Token booking ke liye login karna anivarya hai.
            </p>
            <button onClick={() => navigate('/login')}
              className="w-full py-4 bg-orange-600 text-white rounded-2xl font-bold text-lg shadow-lg hover:bg-orange-700 transition-all flex items-center justify-center gap-3">
              Login Now <LogIn className="w-5 h-5" />
            </button>
            <p className="mt-6 text-sm text-gray-400">
              Naya account? <button onClick={() => navigate('/register')} className="text-orange-600 font-bold hover:underline">Register Karein</button>
            </p>
          </div>

        ) : createdTokenNumber ? (
          <div className="bg-green-50 border border-green-100 p-8 rounded-[2.5rem] text-center shadow-lg">
            ...
            <p className="mt-8 text-sm text-gray-500 italic">
              ✅ Confirmation email bhej di gayi hai aapke registered email pe.
            </p>
            <button onClick={() => { dispatch(clearCreatedBookingState()); setFormData(prev => ({ ...prev, notes: '' })); }}
          ...
          <form className="mt-4 space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-5">
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <input required type="text" name="devoteeName" value={formData.devoteeName} onChange={handleChange}
                    className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-orange-500 transition-all outline-none font-bold text-gray-800"
              ...
              {/* ✅ Email field — auto-filled, read-only if logged in */}
              <div className="group">
                <label className="block text-gray-500 text-xs font-black uppercase tracking-widest mb-1.5 ml-1">Email</label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm">@</span>
                  <input type="email" name="email" value={formData.email} onChange={handleChange}
                    readOnly={isAuthenticated && !!user?.email}
                    className={`w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-orange-500 transition-all outline-none font-bold text-gray-800 ${isAuthenticated && user?.email ? 'opacity-70 cursor-not-allowed' : ''}`}
                    placeholder="Email address" />
                </div>
                {isAuthenticated && user?.email && (
                  <p className="text-xs text-gray-400 mt-1 ml-1">✅ Aapke account ki email use ho rahi hai</p>
                )}
              </div>
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <input required type="number" min="1" name="numberOfPeople" value={formData.numberOfPeople} onChange={handleChange}
                    className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-orange-500 transition-all outline-none font-bold text-gray-800" />
                </div>
              </div>
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <textarea name="notes" value={formData.notes} onChange={handleChange} rows={3}
                    className="w-full pl-12 pr-4 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:bg-white focus:ring-4 focus:ring-orange-100 focus:border-orange-500 transition-all outline-none font-bold text-gray-800 resize-none"
            ...
            <button disabled={loading} type="submit"
              className="w-full py-5 bg-gradient-to-r from-orange-600 to-orange-500 text-white rounded-2xl font-bold text-lg shadow-lg shadow-orange-100 hover:from-orange-700 hover:to-orange-600 hover:-translate-y-1 active:scale-95 transition-all disabled:opacity-50 disabled:transform-none">
