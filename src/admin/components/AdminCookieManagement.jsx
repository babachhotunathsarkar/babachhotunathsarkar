...
import { fetchCookiePolicy, updateCookiePolicyAction, resetCookieState } from '../../redux/cookie/cookieSlice';
import { Cookie, Save, Loader2, CheckCircle, X, Edit, Eye } from 'lucide-react';

export default function AdminCookieManagement() {
    const dispatch = useDispatch();
    const { cookie, loading, error, success } = useSelector(s => s.cookie);
    ...
    useEffect(() => {
        if (success || error) {
            const t = setTimeout(() => dispatch(resetCookieState()), 3000);
            return () => clearTimeout(t);
        }
    }, [success, error, dispatch]);

    const handleSave = async () => {
        await dispatch(updateCookiePolicyAction({ title, content }));
        setIsEditing(false);
    };
        ...
        <div className="p-8 space-y-8 max-w-5xl mx-auto">
            <div className="flex items-center justify-between bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-gray-900">Cookie Policy</h1>
                        <p className="text-gray-500 text-sm font-medium tracking-tight">Manage browser tracking protocols.</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <a href="/cookie-policy" target="_blank" className="flex items-center gap-2 px-6 py-3 border border-gray-200 hover:bg-gray-50 text-gray-700 rounded-xl font-bold transition-all">
            ...
            {success && (
                <div className="flex items-center gap-2 bg-green-500 text-white p-4 rounded-2xl animate-in fade-in slide-in-from-top-2">
                    <CheckCircle className="w-5 h-5" /> <span className="font-bold">Cookie Policy updated successfully!</span>
                </div>
            )}
            {error && (
            ...
            <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-gray-100">
                {!isEditing ? (
                    <div className="space-y-6">
                        <h2 className="text-3xl font-black text-gray-900">{cookie?.title}</h2>
                        <div className="prose prose-orange max-w-none text-gray-500 whitespace-pre-line text-lg leading-relaxed">
                            {cookie?.content || 'No policy content added yet.'}
                        </div>
                    </div>
                ) : (
