...
import { fetchPrivacyPolicy, updatePrivacyPolicyAction, resetPrivacyState } from '../../redux/privacy/privacySlice';
import { Shield, Save, Loader2, CheckCircle, X, Edit, Eye } from 'lucide-react';

export default function AdminPrivacyManagement() {
    const dispatch = useDispatch();
    const { privacy, loading, error, success } = useSelector(s => s.privacy);
    ...
    useEffect(() => {
        if (success || error) {
            const t = setTimeout(() => dispatch(resetPrivacyState()), 3000);
            return () => clearTimeout(t);
        }
    }, [success, error, dispatch]);

    const handleSave = async () => {
        await dispatch(updatePrivacyPolicyAction({ title, content }));
        setIsEditing(false);
    };
        ...
        <div className="p-8 space-y-8 max-w-5xl mx-auto">
            <div className="flex items-center justify-between bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-gray-500 text-sm font-medium tracking-tight">Manage user data protection rules.</p>
                    </div>
                </div>
                <div className="flex gap-2">
                    <a href="/privacy-policy" target="_blank" className="flex items-center gap-2 px-6 py-3 border border-gray-200 hover:bg-gray-50 text-gray-700 rounded-xl font-bold transition-all">
            ...
            {success && (
                <div className="flex items-center gap-2 bg-green-500 text-white p-4 rounded-2xl animate-in fade-in slide-in-from-top-2">
                    <CheckCircle className="w-5 h-5" /> <span className="font-bold">Privacy Policy updated successfully!</span>
                </div>
            )}
            {error && (
            ...
            <div className="bg-white rounded-[2rem] p-10 shadow-sm border border-gray-100">
                {!isEditing ? (
                    <div className="space-y-6">
                        <h2 className="text-3xl font-black text-gray-900">{privacy?.title}</h2>
                        <div className="prose prose-orange max-w-none text-gray-500 whitespace-pre-line text-lg leading-relaxed">
                            {privacy?.content || 'No policy content added yet.'}
                        </div>
                    </div>
                ) : (
