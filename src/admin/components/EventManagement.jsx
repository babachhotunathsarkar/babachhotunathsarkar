...
import {
  ...
  RefreshCw, MapPin, Clock, Search, ToggleLeft, ToggleRight,
} from 'lucide-react';
...
export default function EventManagement() {
  ...
  const pagination = eventsState?.pagination ?? {
    currentPage: 1,
    totalPages: 1,
    totalItems: 0,
    itemsPerPage: 10
  ...
  useEffect(() => {
    ...
    loadEvents();
  }, [pagination.currentPage, debouncedSearch, dispatch, pagination.itemsPerPage]);
  ...
  }, [dispatch, pagination.currentPage, pagination.itemsPerPage, debouncedSearch]);
  ...
  const openEdit = (item) => {
    ...
    if (item.eventDate) {
      const datePart = new Date(item.eventDate).toISOString().split('T')[0];
      const timePart = item.eventTime || '12:00';
      let formattedTime = '12:00';

      if (timePart.match(/^\d{1,2}:\d{2}$/)) {
        formattedTime = timePart;
      } else if (timePart.match(/(\d{1,2}):(\d{2})\s*(AM|PM)/i)) {
      ...
      eventDateTime = `${datePart}T${formattedTime}`;
    }
  ...
  const handleSubmit = async () => {
    ...
    const formattedTime = eventTime ? formatTo12Hour(eventTime) : '';
    ...
    try {
      if (editItem) {
        await dispatch(updateEvent({ id: editItem._id, data: submitData })).unwrap();
  ...
  const handleToggle = async (id) => {
    try {
      await dispatch(toggleEvent(id)).unwrap();
      refetch();
    } catch (err) {
  ...
  const activeCount = Array.isArray(events) ? events.filter(e => e?.isActive).length : 0;
  const upcomingCount = Array.isArray(events) ? events.filter(e => e?.eventDate && new Date(e.eventDate) >= new Date()).length : 0;
    ...
    <div className="min-h-screen bg-[#FAFAF7]" style={{ fontFamily: "'Cormorant Garamond', Georgia, serif" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: 'Total Events', value: pagination?.totalItems || 0, color: 'text-[#581c87]' },
            { label: 'Active', value: activeCount, color: 'text-emerald-600' },
            { label: 'This Page', value: events?.length || 0, color: 'text-violet-600' },
            { label: 'Upcoming', value: upcomingCount, color: 'text-sky-600' },
          ].map(s => (
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-3">
              ...
              {debouncedSearch && (
                <button
                  onClick={() => { setSearch(''); }}
                  className="text-xs px-4 py-2 bg-violet-50 text-violet-700 rounded-xl font-sans hover:bg-violet-100"
                >
                  Clear filters
                </button>
              )}
                    ...
                    {events.map((event) => (
                      <tr key={event._id} className="hover:bg-violet-50/30 transition-colors">
                        <td className="px-4 py-4">
                          ...
                          {event.description && (
                            <p className="text-[11px] text-gray-400 font-sans truncate max-w-[200px] mt-0.5">
                              {event.description}
                            </p>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-4">
                          <button
                            ...
                            className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-sans font-medium transition-colors ${event.isActive
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-1">
                            ...
                            <button
                              onClick={() => { setDeleteId(event._id); setShowDelete(true); }}
                ...
                {events.map((event) => (
                  <div key={event._id} className="p-4">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button
                        onClick={() => handleToggle(event._id)}
                        className={`text-[11px] px-3 py-1.5 rounded-lg font-sans font-medium border ${event.isActive
                      ...
                      <button
                        onClick={() => openEdit(event)}
                        className="text-[11px] px-3 py-1.5 rounded-lg bg-violet-50 text-violet-700 font-sans font-medium border border-violet-100"
                      >
                        Edit
                      </button>
                      <button
          ...
          {pagination?.totalPages > 1 && (
            <div className="flex flex-wrap justify-between items-center px-5 py-4 border-t border-stone-100 gap-3">
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => dispatch(setEventPage(pagination.currentPage - 1))}
                ...
                {Array.from({ length: pagination.totalPages }, (_, i) => i + 1)
                  ...
                  .map((p, i) => (
                    p === '...' ? (
                      ...
                      <button
                        key={p}
                        onClick={() => dispatch(setEventPage(p))}
                        className={`w-8 h-8 rounded-lg text-sm font-sans font-medium transition-all ${pagination.currentPage === p
                            ? 'bg-gradient-to-b from-[#7e22ce] to-[#581c87] text-white shadow-sm'
        ...
        >
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full sm:max-w-lg shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest font-sans mb-1.5">
                  Title *
                </label>
                <input
                  type="text"
              ...
              <div className="flex gap-3 pt-1">
                <button
                  onClick={() => setShowForm(false)}
                  className="flex-1 py-2.5 text-sm border border-stone-200 rounded-xl hover:bg-stone-50 font-sans text-gray-700"
                >
                  Cancel
                </button>
                <button
          ...
          <div className="bg-white rounded-3xl max-w-sm w-full p-7 shadow-2xl border border-red-100">
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 py-2.5 text-sm border border-stone-200 rounded-xl hover:bg-stone-50 font-sans"
              >
                Cancel
              </button>
              <button
                onClick={handleDelete}
                className="flex-1 py-2.5 text-sm bg-red-500 text-white rounded-xl hover:bg-red-600 font-sans font-semibold"
              >
                Delete
              </button>
            </div>
